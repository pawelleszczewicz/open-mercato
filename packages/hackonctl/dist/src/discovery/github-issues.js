async function fetchOpenIssues(github, config) {
  const allIssues = [];
  let page = 1;
  const perPage = 100;
  while (true) {
    const issues = await github.listIssues(
      ...
      {
        state: "open",
        labels: config.discovery.issueLabels.length > 0 ? config.discovery.issueLabels.join(",") : void 0,
        per_page: perPage,
        page
      }
    );
...
function issuesToCandidates(issues) {
  return issues.filter((issue) => !issue.assignee).map((issue) => {
